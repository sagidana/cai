...
UserConfig.load() imports every extension's Python once, so the cai.hook /
cai.command decorators bake what they register into the process-global
HooksRegistry and CommandsRegistry. UserConfig.skill_dirs() / tool_dirs() are the
filesystem search paths skills.py and tools.py consult before the builtins.
UserConfig.extension_for() maps a registered hook/command (by the file it was
defined in) back to its extension."""
...
class UserConfig:
    ...
    def load():
        ...
        top-level init.py, so their cai.hook / cai.command decorators register
        into the global HooksRegistry / CommandsRegistry. returns the discovered
        extensions and user settings; the hooks and commands live in those
        registries."""
